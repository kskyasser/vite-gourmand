...
$commande_id = isset($_GET['commande_id']) ? (int)$_GET['commande_id'] : (int)($_GET['id'] ?? 0);
  if ($commande_id <= 0) {
  ...
  // Sécurité : un utilisateur ne peut voir QUE ses commandes (employe/admin peuvent tout voir)
  ...
  // Dernier statut (table: commandes_statuts)
  $stmt = $pdo->prepare("
